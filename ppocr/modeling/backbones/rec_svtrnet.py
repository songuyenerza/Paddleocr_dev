...
class SVTRNet(nn.Layer):
    ...
    # def hardswish_alter(self, x):
    #     # Simplify the computation by replacing division with multiplication
    #     # Ensure the multiplier is of the same dtype as x for consistency
    #     # Clip the values using paddle.clip, which should handle FP16 inputs appropriately
    #     clipped = paddle.clip(x + 3.0, min=0.0, max=6.0)
    #     # Perform the final operation

    #     # The dtype of the output will automatically match that of x
    #     return x * clipped * 0.16666666667
    ...
    def forward(self, x):
        ...
        if self.use_lenhead:
            ...
            len_x = self.dropout_len(self.ReLU_len(len_x))
            print('---------------------relu-------------------------')
