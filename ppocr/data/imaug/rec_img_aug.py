...
#   todo: sonnt
def Aug_custum_mouse(image):
    # Get image dimensions
    height, width = image.shape[:2]
    square_size = int(height/10)
    # Create a mask with the same size as the image
    mask = np.zeros((height, width), dtype=np.uint8)

    # Generate random coordinates for the squares
    # coordinates = np.random.randint(0, min(height, width) - square_size, size=(n, 2))

    # Iterate over the coordinates and draw squares on the mask
    for _ in range(int(width/random.randint(16, 25))):
        x = random.randint(0, width) - square_size
        y = random.randint(0, height) - square_size

        mask[y:y+square_size, x:x+square_size] = 255

    # Get the background color of the image
    background_color = image[0, 0]
    # Create a copy of the image
    result_image = image.copy()
    # Replace pixels in the image with the background color where the mask is white
    result_image[mask == 255] = background_color
...
class RecAug(object):
    def __init__(self,
        ...
        self.transform = A.Compose([
            ...
            A.JpegCompression(quality_lower=10, quality_upper=100, p=0.2),
        ])
    ...
    def __call__(self, data):
        ...
        try:
            ...
            print("Error in BaseDataAugmentation:", str(e))
